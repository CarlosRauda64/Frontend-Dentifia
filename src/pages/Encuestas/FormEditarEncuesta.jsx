...
const FormEditarEncuesta = () => {
    ...
    useEffect(() => {
        const fetchEncuesta = async () => {
            try {
                const response = await fetch(`${API_URL}/encuestas/encuestas/${id}/`, {
                    headers: {
                ...
                if (response.ok) {
                    ...
                    setValue("nivel_satisfaccion", data.nivel_satisfaccion || 0);
                    setValue("observaciones", data.observaciones || '');
                    
                    // Convertir preguntas_respuestas (objeto) a array para el formulario
                    if (data.preguntas_respuestas && typeof data.preguntas_respuestas === 'object') {
                        const preguntasArray = Object.entries(data.preguntas_respuestas).map(([pregunta, respuesta]) => ({
                            pregunta,
                            respuesta
                        }));
                        replace(preguntasArray);
                    }
                    
                    reset(data);
                } else {
                    console.error("Error al obtener datos de la encuesta.");
                }
            } catch (error) {
        ...
        fetchEncuesta();
    }, [id, reset, setValue, replace, auth]);

    const editarEncuesta = async (data) => {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(errorData.error || 'Error al editar la encuesta');
            } else {
                navigate('/encuestas');
            }
        } catch (error) {
            console.error('Error al editar la encuesta:', error);
            alert(error.message || 'Error al editar la encuesta. Por favor, intente nuevamente.');
        }
    };

    const onSubmit = async (data) => {
        await editarEncuesta(data);
    };
    ...
    return (
        <Navegacion>
            <div className="flex flex-col items-center sm:justify-center w-full h-full max-sm:mt-10">
                <h1 className="text-2xl font-bold mb-4 text-black dark:text-white">Editar Encuesta</h1>
                <form className="flex flex-col gap-4 dark:bg-gray-800 bg-white p-10 rounded-2xl w-[75%] max-w-2xl" onSubmit={handleSubmit(onSubmit)}>
                    ...
                    <div className='flex gap-2 justify-evenly items-center col-span-2'>
                        <Button type="submit" className="mt-4">Actualizar Encuesta</Button>
...
export default FormEditarEncuesta;
